...
struct link_list{
    ...
    struct link_list *add_prev_node;
};
...
void creation(){
    ...
    for(int i=0; i<n ;i++){
        if(i==0){
            ...
            start = pointer;
            pointer->add_prev_node = NULL;
        }
        else{
            pointer->add_next_node = malloc(sizeof(struct link_list));
            pointer->add_next_node->add_prev_node = pointer;
...
void insert(){
    ...
    switch(x){
        case 1:
            pointer = start;
            pointer->add_prev_node = malloc(sizeof(struct link_list));
            start = pointer->add_prev_node;
            pointer->add_prev_node->add_next_node = pointer;
            pointer = start;
            printf("Enter the value : ");
            scanf("%d",&pointer->value);
            pointer -> add_prev_node = NULL;
...
void deletion(){
    ...
    switch(x){
        ...
        case 3:
            ...
            pointer->add_next_node->add_prev_node = prev_point;
            printf("%d is deleted\n", pointer->value);
            free(pointer);
            break;
    }
